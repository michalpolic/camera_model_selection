...
function [cameras, images, points3D] = read_model(path, database_file)
...
dataset_mat_file = fullfile(path,'dataset.mat');
if exist(dataset_mat_file,'file')
    load(dataset_mat_file);
    return;
end

cameras = read_cameras([path 'cameras.txt']);
if nargin > 1
    images = read_images([path 'images.txt'],database_file);
else
...
while ischar(tline)
    ...
    camera.params = zeros(numel(elems) - 4, 1);
    for i = 5:numel(elems)
...
function images = read_images(path,database_file)

images = containers.Map('KeyType', 'int64', 'ValueType', 'any');
if nargin > 1
   conn = database(database_file,[],[],'org.sqlite.JDBC',sprintf('jdbc:sqlite:%s',database_file));
end
...
while ischar(tline)
    ...
    image.q = [qw, qx, qy, qz];
    image.q = image.q / norm(image.q);
    image.R = quat2rotmat(image.q);
    ...
    % if possible load the SIFT scale from database
    if nargin > 1
        data = fetch(conn,sprintf('SELECT rows,cols,data FROM keypoints WHERE image_id = %d',image.image_id));
        keypoints = reshape(typecast( data{3} , 'single'),data{2},data{1});
        if size(image.xys,1) == data{1}
            if data{2} == 4         % x, y, scale, orientation
                image.xys_scale = keypoints(3,:);
            elseif data{2} == 6     % x, y, a11, a21, a12, a22
                image.xys_afine = keypoints(3:6,:);
                image.xys_scale = arrayfun(@(c) max(abs(eig(reshape(keypoints(3:6,c),2,2)))),1:data{1});
            end
        else
           error('The database does not match the txt files.'); 
        end
    end
...
while ischar(tline)
    ...
    point = struct;
    point.point3D_id = double(elems(1));
    point.xyz = elems(2:4);
